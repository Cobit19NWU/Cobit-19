...
namespace Cobit_19.Business.ObjectiveAudits
{
    public class ObjectiveAuditProvider
    {
        ...
        public ObjectiveDto getObjective(int ObjectiveAuditID)
        {
            var ObjectiveAudit = getByID(ObjectiveAuditID);
            var objectiveID = ObjectiveAudit.ObjectiveID;

            var objectiveQuery = _dbContext.Objectives.Where(obj => obj.ID == objectiveID).FirstOrDefault();

            return _mapper.Map<ObjectiveDto>(objectiveQuery);
        }
        ...
        public double calculateAnswerScore(int answerValue)
        {
            ...
            {
                return Math.Round(answerValue * 0.66, 2);
            }
            else
            {
                return Math.Round(answerValue * 1.00, 2);
            }
        }
        ...
        public double calculateFinalMaturityLevelPercs(FullObjectiveAuditDto fullAudit, string MLevel)
        {
            ...
            double totalMaturityLevelScore = 0;

            foreach (SubComponentDto subComp in processSubComps)
            {
                List<SubComponentQuestionDto> MLevelQuestions = GetSubComponentMLevelQuestions(subComp, MLevel);

                if (MLevelQuestions != null && MLevelQuestions.Count > 0)
                {
                    foreach (SubComponentQuestionDto question in MLevelQuestions)
                    {
                        totalMaturityLevelScore += question.questionScore;
                        totalQuestionCount++;
                    }
                }
            }
            ...
            if (totalQuestionCount > 0)
            {
                ...
                double finalMaturityPerc = totalMaturityLevelScore / totalQuestionCount;
        ...
        public int CalculateFinalMaturityLevel(FullObjectiveAuditDto fullAudit)
        {
            ...
            else if (level5Perc >= 0.99 && level5Perc < 1.60 &&
                     level4Perc >= 0.99 && level4Perc < 1.60 &&
                     level3Perc >= 0.99 && level3Perc < 1.60 &&
                     level2Perc >= 0.99 && level2Perc < 1.60 ||
                     level3Perc >= 0.99 && level3Perc < 1.60 &&
                     level2Perc >= 0.99 && level2Perc < 1.60)
            {
